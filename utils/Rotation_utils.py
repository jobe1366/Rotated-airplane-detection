...
def preprocess(image_path, max_size=768, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ori_imgs = [cv2.imread(img_path) for img_path in image_path]

    normalized_imgs = [ (img[..., ::-1] / 255.0 - mean) / std     for img in ori_imgs ]
...
# for coco-eval
def eval_preprocess(*image_path, max_size=768, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ori_imgs = [cv2.imread(img_path) for img_path in image_path]
    normalized_imgs = [  (img[..., ::-1] / 255.0 - mean) / std   for img in ori_imgs]
...
def preprocess_video(*frame_from_video, max_size=768, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
...
def display(preds, imgs, obj_list, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        ...
        if imwrite:
            ...
            cv2.imwrite(f'test/{uuid.uuid4().hex}.png', imgs[i])
####################################################################################################################################
