...
# Batch detection pictures (used to generate evaluation files, only suitable for len(ratios) * len(scales) = 9
# Operation mode:
# Step 1:
# The file generated by this code is in the /home/fzh/Rotation-EfficinetDet/Evaluation_on_val_set/result_classname/ folder
# Under the path corresponding to path in the write_into_txt() function
#
# Step 2:
...
# file tree:
# The location of the generated txt file corresponding to path in the write_into_txt() function
# path: Represents storing the basename of the image to be detected in a txt file
# imgpath: Represents the path where the image to be detected is stored The stored image is in *.png format
#
# line 239 write_into_txt(file_name, filecontent) is used to write the test results to a file
# line 241 display(filebasename, out, ori_imgs, imshow=False, imwrite=True) is used to save the test result picture 
