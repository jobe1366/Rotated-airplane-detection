...
def get_args():
    ...
    parser.add_argument('-p', '--project', type=str, default='DOTA_effdet', help='project file that contains parameters')
    ...
    parser.add_argument('--val_batch_size', type=int, default=  84, help='the number of images per batch for val')
    parser.add_argument('--head_only', type=boolean_string, default= False,
                        help='whether finetunes only the regressor and the classifier, '
                             'useful in early stage convergence or small/easy dataset')
    parser.add_argument('--init_head', type=boolean_string, default= False,
    ...
    parser.add_argument('--optim', type=str, default= 'sgd', help='select optimizer for training, '
                                                                   ...
                                                                   ' very final stage then switch to \'sgd\'') #'adamw'
    ...
    parser.add_argument('--data_path', type=str, default='/home/jobe/Rotated_airplane/datasets/DOTA_effdet', help='the root folder of dataset')
...
def train(opt):
    ...
    training_set = RotationCocoDataset(root_dir=os.path.join(opt.data_path), set=params.train_set,
                                       transform=transforms.Compose([Normalizer(mean=params.mean, std=params.std),
                                                                     Resizer(input_sizes[opt.compound_coef])]))
    training_generator = DataLoader(training_set, **training_params)


    val_set = RotationCocoDataset(root_dir=os.path.join(opt.data_path), set=params.val_set,
                                  transform=transforms.Compose([Normalizer(mean=params.mean, std=params.std),
                                                                Resizer(input_sizes[opt.compound_coef])]))
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=3,  min_lr= 1e-5,  verbose=True)
            ...
            for iter, data in enumerate(progress_bar):
                ...
                try:
                    ...
                    progress_bar.set_description(
                        'Step: {}. Epoch: {}/{}. Iteration: {}/{}. Cls loss: {:.4f}. Reg loss: {:.4f}. Total loss: {:.4f}'.format(
